...
function MovieCard({ movie, isFav, style, disableHover }) {
    ...
    return (
        <div style={style}>
            <Link to={`/movie/${movie.id}`}>
                <div
                    ref={cardRef}
                    className={`relative shrink-0 cursor-pointer rounded-2xl bg-white shadow-md transition-all ${!disableHover ? 'hover:scale-[1.015] hover:shadow-xl' : ''}`}
                    style={{
                        width: CARD_WIDTH,
                        height: CARD_HEIGHT,
                        margin: MARGIN,
            ...
            <div className="btn-fav absolute z-30 text-white" style={{ pointerEvents: 'auto' }}>
                {isFav ?
